...
OAuth.registerService('battlenet', 2, null, function(query) {
  ...
  var bnet = getBNetTag(accessToken);
  var admin = false;
...
var getCharacters = function (accessToken) {
  try {
    ...
    console.log("Error Grabbing Characters");
    return {};
  }
};

var getBNetTag = function (accessToken) {
  try {
    return Meteor.http.get("https://us.api.battle.net/account/user", {
      params: { access_token: accessToken } }
    ).data;
  } catch (err) {
    throw new Error("Failed to fetch identity from Battlenet (Tag). " + err.message);
  }
}
